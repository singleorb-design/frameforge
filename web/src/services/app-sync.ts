...
import { createWorkbenchRecordStore } from "@/services/workbench-records";
...
const imageLogStore = createWorkbenchRecordStore("image");
const videoLogStore = createWorkbenchRecordStore("video");
    ...
    const [canvas, assets, imageLogs, videoLogs] = await Promise.all([
        syncDomain<CanvasDomainData>(config, onProgress, {
            ...
            localData: async () => {
                ...
                const projects = (await Promise.all(store.projects.map((project) => store.openProject(project.id, false)))).filter((project): project is CanvasProject => Boolean(project));
                return { projects };
            },
        ...
        syncDomain<AssetDomainData>(config, onProgress, {
            ...
            applyData: async (data) => void (await useAssetStore.getState().replaceAssets(await Promise.all(data.assets.map(hydrateAsset)))),
        }),
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        const manifest: DomainManifest<T> = { app: "frameforge", version: 1, domain: options.key, exportedAt: new Date().toISOString(), data: mergedData, files: uploaded.files };
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    ...
    if (data.app !== "frameforge" || data.domain !== domain) throw new Error(`${domain} 同步清单不是当前应用的数据`);
    return {
        app: "frameforge",
        version: 1,
        domain,
...
async function downloadMissingFiles<T>(config: WebdavSyncConfig, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async (remoteFile) => {
        ...
        const scope = syncMediaScope(domain, data, remoteFile.storageKey);
        await (remoteFile.storageKey.startsWith("image:") ? setImageBlob(remoteFile.storageKey, typedBlob, scope) : setMediaBlob(remoteFile.storageKey, typedBlob, scope));
        downloaded += 1;
...
function syncMediaScope(domain: DomainKey, data: unknown, storageKey: string): StorageScope {
    if (domain === "assets") return { kind: "library", ownerId: "library" };
    if (domain === "image-workbench") return { kind: "workbench", ownerId: "image" };
    if (domain === "video-workbench") return { kind: "workbench", ownerId: "video" };
    const projects = (data as CanvasDomainData).projects || [];
    const project = projects.find((item) => JSON.stringify(item).includes(JSON.stringify(storageKey)));
    if (!project) throw new Error(`同步媒体 ${storageKey} 找不到所属画布`);
