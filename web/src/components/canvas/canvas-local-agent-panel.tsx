...
const AGENT_CONNECT_STEPS = [
    { title: "准备 Hermes", text: "先在本机安装并完成 Hermes 配置，确认终端可以执行 `hermes acp --check`。" },
    { title: "启动 Canvas Agent", text: "在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", command: "npx -y @basketikun/canvas-agent" },
];
...
type AgentHelloEvent = { ok?: boolean; clientId?: string; agent?: AgentRuntimeState };
...
export function CanvasLocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    const loadThreadsRef = useRef<(skipHistory?: boolean) => Promise<void>>(async () => {});
    ...
    const loadThreads = useCallback(async (skipHistory = false) => {
        ...
        try {
            const data = await fetchAgentJson<AgentThreadsResponse>(endpoint, token, `/agent/hermes/threads`);
            const nextThreadId = data.workspace?.activeThreadId || "";
            let nextMessages: AgentChatItem[] = skipHistory ? useAgentStore.getState().messages : [];
            if (nextThreadId && !skipHistory) {
                const thread = await fetchAgentJson<AgentThreadResponse>(endpoint, token, `/agent/hermes/threads/${encodeURIComponent(nextThreadId)}`);
    ...
    useEffect(() => {
        ...
        source.addEventListener("hello", (event) => {
            const busy = Boolean(parseEventData<AgentHelloEvent>(event)?.agent?.busy);
            errorLoggedRef.current = false;
            connectedRef.current = true;
            setAgentState({ connected: true, activity: busy ? "Hermes 正在运行" : "已连接", waiting: busy, sending: false, connectError: "", silentConnect: false, messages: useAgentStore.getState().messages.filter((item) => !isConnectionErrorMessage(item)) });
        ...
        source.addEventListener("agent_state", (event) => {
            ...
            enqueueEvent(async () => {
                ...
                setAgentState({ activity: busy ? "Hermes 正在运行" : "完成", waiting: busy, ...(busy ? {} : { sending: false }) });
                if (!busy) await loadThreadsRef.current(true);
            });
        });
        ...
        source.addEventListener("workspace_changed", (event) => {
            ...
            enqueueEvent(async () => {
                ...
                await loadThreadsRef.current(data.emptyThread);
            });
        });
        ...
        source.onerror = () => {
            ...
            if (!errorLoggedRef.current || wasConnected) {
                ...
                if (!headlessRef.current && !silent) messageRef.current.error(text);
            }
    ...
    const sendPrompt = async () => {
        ...
        try {
            const data = await fetchAgentJson<{ threadId?: string }>(endpoint, token, "/agent/hermes/turn", {
                method: "POST",
            ...
            const busy = text.includes("Hermes 正在运行");
            setAgentState({ activity: busy ? "Hermes 正在运行" : "发送失败" });
    ...
    const stopTurn = async () => {
        ...
        try {
            await fetch(`${endpoint}/agent/hermes/interrupt?token=${encodeURIComponent(token)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ threadId: useAgentStore.getState().activeThreadId || undefined }) });
            addEventLog("用户停止", {});
        } catch {
    ...
    const toggleAgentConnection = async ({ silent = false }: { silent?: boolean } = {}) => {
        ...
        if (!nextToken) {
            const text = discovered?.hasToken ? "已发现本地 Agent，但未读取到 Connect token，请重启 Canvas Agent 或手动填写 token" : "没有发现本地 Agent，请先手动启动 Canvas Agent";
            if (!silent) {
                setAgentState({ connectError: text });
                if (!headless) message.warning(text);
            }
    ...
    const startNewThread = async () => {
        ...
        setAgentState({ loadingThreads: true });
        try {
            const data = await fetchAgentJson<AgentThreadResponse>(endpoint, token, "/agent/hermes/threads/new", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({}) });
    ...
    const resumeThread = async (threadId: string) => {
        ...
        setAgentState({ loadingThreads: true });
        try {
            const data = await fetchAgentJson<AgentThreadResponse>(endpoint, token, `/agent/hermes/threads/${encodeURIComponent(threadId)}/resume`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({}) });
    ...
    const deleteThread = async (threadId: string) => {
        ...
        setAgentState({ loadingThreads: true });
        try {
            await fetchAgentJson(endpoint, token, `/agent/hermes/threads/${encodeURIComponent(threadId)}/delete`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({}) });
            ...
            {activeTab === "setup" ? (
                ...
                <>
                    ...
                    <AgentChatComposer
                        ...
                        placeholder="询问 Hermes，或让它操作网站/画布"
                        theme={theme}
...
function AgentConnectView({
    ...
    const hermesReminder = (
        <div className="rounded-lg border px-3 py-2.5 text-xs leading-5" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
            ...
            <div className="mt-1">网页连接的是本机 Canvas Agent；Canvas Agent 会在收到任务时启动 `hermes acp`，并把 FrameForge MCP 工具注入 Hermes 会话。</div>
        </div>
    );
    return (
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
...
function formatAgentEvent(event: AgentEventPayload): Omit<AgentChatItem, "id"> | null {
    ...
    if ((event.type === "item.updated" || event.type === "item.completed") && item?.type === "agent_message") return { role: "assistant", title: "Hermes", text: stringText(item.text), meta: usageText(event), streamId: item.id };
    ...
    if (text) return { role: "assistant", title: "Hermes", text, meta: usageText(event) };
    return null;
}
...
function eventTitle(event: AgentEventPayload) {
    const item = event.item;
    if (event.type === "thread.started") return "已创建 Hermes 会话";
    ...
    if (event.type === "item.completed" && item?.type === "agent_message") return "Hermes 回复";
    return event.type || "Hermes 事件";
}
