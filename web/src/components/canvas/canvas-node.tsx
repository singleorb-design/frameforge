...
type CanvasNodeProps = {
    ...
    onToggleCharacterGroup?: (nodeId: string) => void;
    onToggleCharacterAssetGroup?: (nodeId: string) => void;
    onSetBatchPrimary?: (node: CanvasNodeData) => void;
    onRetry?: (node: CanvasNodeData) => void;
    onGenerateImage?: (node: CanvasNodeData) => void;
    onViewImage?: (node: CanvasNodeData) => void;
    onOpenMarkdownReader?: (node: CanvasNodeData) => void;
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    onTitleChange,
    onToggleBatch,
    onToggleCharacterGroup,
    onToggleCharacterAssetGroup,
    onSetBatchPrimary,
    onRetry,
    onGenerateImage,
    onViewImage,
    onOpenMarkdownReader,
    onContextMenu,
}: CanvasNodeProps) {
        ...
        >
            ...
            <div
                ...
                onDoubleClick={(event) => {
                    ...
                    if (data.type === CanvasNodeType.CharacterGroup) {
                        event.stopPropagation();
                        onToggleCharacterGroup?.(data.id);
                        return;
                    }
                    if (data.type === CanvasNodeType.CharacterAssetGroup) {
                        event.stopPropagation();
                        onToggleCharacterAssetGroup?.(data.id);
                        return;
                    }
                    ...
                    if (data.type === CanvasNodeType.Script || data.type === CanvasNodeType.MarkdownDocument) {
                        event.stopPropagation();
                        onOpenMarkdownReader?.(data);
                        return;
                    }
                    if (data.type === CanvasNodeType.CharacterCard) {
                        event.stopPropagation();
                        return;
                    }
                    ...
                    style={
                        {
                            ...
                            animation:
                                data.type === CanvasNodeType.Image && (data.metadata?.batchRootId || data.metadata?.characterAssetRootId)
                                    ? batchClosing
                                        ? "canvas-batch-child-out 260ms cubic-bezier(.4,0,.2,1) both"
                                        : "canvas-batch-child-in 340ms cubic-bezier(.2,.85,.18,1) both"
                                    : data.type === CanvasNodeType.CharacterCard && batchMotion
                                      ? batchClosing
                                          ? "canvas-batch-child-out 260ms cubic-bezier(.4,0,.2,1) both"
                                          : "canvas-batch-child-in 340ms cubic-bezier(.2,.85,.18,1) both"
                                      : undefined,
                            animationDelay:
                                (data.type === CanvasNodeType.Image && (data.metadata?.batchRootId || data.metadata?.characterAssetRootId)) || (data.type === CanvasNodeType.CharacterCard && batchMotion)
