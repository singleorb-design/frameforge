...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    return (
        <div className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }} onWheel={(event) => event.stopPropagation()}>
            ...
            {mode === "text" ? (
                <div className="mb-2 flex cursor-default items-center justify-between gap-2 text-[11px]" onMouseDown={(event) => event.stopPropagation()}>
                    <span style={{ color: theme.node.muted }}>输出</span>
                    <Segmented
                        size="small"
                        className="canvas-config-mode !rounded-md !p-0.5"
                        value={node.metadata?.textOutputType || "text"}
                        onChange={(value) => onConfigChange(node.id, { textOutputType: value as CanvasNodeMetadata["textOutputType"] })}
                        options={[
                            { value: "text", label: "普通文本" },
                            { value: "markdown", label: "Markdown 文档" },
                            {
                                value: "shotboard",
                                label: <span title="需要直接连接剧本；角色设定、场景卡组和道具卡组可作为素材目录输入">结构化分镜</span>,
                            },
                        ]}
                    />
                </div>
            ...
            <Button
                type="primary"
                className="mt-auto !h-9 !w-full !shrink-0 !cursor-pointer !rounded-lg"
                danger={isRunning}
