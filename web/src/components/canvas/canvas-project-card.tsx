...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
    ...
    const loaded = useCanvasStore((state) => state.loadedProjectIds.has(project.id));
    ...
    const exportProject = async () => {
        const fullProject = await openProject(project.id, false);
        if (fullProject) await exportCanvasProjects([fullProject], fullProject.title || "影格工坊");
    };
            ...
            <div className="flex items-start gap-3">
                ...
                {editing ? (
                    ...
                    >
                        ...
                        <p className="mt-3 text-sm leading-6 text-stone-600 dark:text-stone-400">{loaded ? `${project.nodes.length} 个节点 · ${project.connections.length} 条连线` : "内容已保存到本地磁盘"}</p>
                    </button>
                )}
            </div>
