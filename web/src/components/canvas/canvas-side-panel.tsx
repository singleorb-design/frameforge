...
import { BookOpen, Check, ChevronRight, Clapperboard, Download, Eye, FileText, FileType, Image as ImageIcon, Images, ListChecks, Map, Music2, Package, Plus, ScrollText, Search, Settings2, Square, Trash2, Type, UsersRound, UserRound, Video } from "lucide-react";
...
const NODE_TYPE_ICON: Record<string, typeof Square> = {
    ...
    [CanvasNodeType.Script]: BookOpen,
    [CanvasNodeType.MarkdownDocument]: FileType,
    [CanvasNodeType.CharacterGroup]: UsersRound,
    [CanvasNodeType.CharacterCard]: UserRound,
    [CanvasNodeType.CharacterAssetGroup]: Images,
    [CanvasNodeType.SceneGroup]: Map,
    [CanvasNodeType.SceneCard]: Map,
    [CanvasNodeType.PropGroup]: Package,
    [CanvasNodeType.PropCard]: Package,
    [CanvasNodeType.Shotboard]: ScrollText,
    [CanvasNodeType.Shot]: Clapperboard,
...
export function CanvasSidePanel({ nodes, production, selectedNodeIds, onFocusNode, onInsertAsset }: Props) {
        ...
        >
            ...
            >
                ...
                <div className="mt-2 min-h-0 flex-1 overflow-hidden">
                    {tab === "canvas" ? (
                        <CanvasNodesTab nodes={nodes} production={production} selectedNodeIds={selectedNodeIds} onFocusNode={onFocusNode} theme={theme} />
                    ) : tab === "assets" ? (
...
const NODE_FILTER_OPTIONS = [
    ...
    { label: "剧本", value: CanvasNodeType.Script },
    { label: "文档", value: CanvasNodeType.MarkdownDocument },
    { label: "角色设定", value: CanvasNodeType.CharacterCard },
    { label: "视觉参考", value: CanvasNodeType.CharacterAssetGroup },
...
function nodePreviewText(node: CanvasNodeData, production: ProductionProject) {
    if (node.type === CanvasNodeType.Text) return textPreview(node.metadata?.content || node.metadata?.prompt || "");
    if (node.type === CanvasNodeType.Script || node.type === CanvasNodeType.MarkdownDocument) return textPreview(node.metadata?.content || "");
    if (node.type === CanvasNodeType.CharacterGroup) return `${node.metadata?.characterChildIds?.length || 0} 个主要角色`;
    if (node.type === CanvasNodeType.CharacterCard) return [node.metadata?.role, node.metadata?.positivePrompt].filter(Boolean).join(" · ");
    if (node.type === CanvasNodeType.CharacterAssetGroup) return `${node.metadata?.characterAssetChildIds?.length || 0} 项视觉参考`;
    if (node.type === CanvasNodeType.SceneGroup || node.type === CanvasNodeType.PropGroup || node.type === CanvasNodeType.Shotboard) return `${node.metadata?.productionChildIds?.length || 0} 个子节点`;
    if (node.type === CanvasNodeType.SceneCard) {
        const card = findProductionVersion(production.scenes, node);
        return card ? `${card.spatialLayout} · ${card.defaultLighting}` : "场景数据缺失";
    }
    if (node.type === CanvasNodeType.PropCard) {
        const card = findProductionVersion(production.props, node);
        return card ? `${card.material} · ${card.scale}` : "道具数据缺失";
    }
    if (node.type === CanvasNodeType.Shot) {
        ...
        return shot ? `${shotStatusLabels[shot.status]} · ${shot.framing.shotSize} · ${shot.startState} -> ${shot.endState}` : "镜头数据缺失";
    }
    return getNodeDefinition(node.type)?.title || node.type;
}

function textPreview(value: string) {
    return value.slice(0, 640).replace(/\s+/g, " ").trim().slice(0, 160);
}

function findProductionVersion<T>(records: Array<{ id: string; versions: Array<{ version: number; data: T }> }>, node: CanvasNodeData) {
    return records.find((record) => record.id === node.metadata?.productionRecordId)?.versions.find((version) => version.version === node.metadata?.productionVersion)?.data;
}

function CanvasNodesTab({ nodes, production, selectedNodeIds, onFocusNode, theme }: { nodes: CanvasNodeData[]; production: ProductionProject; selectedNodeIds: Set<string>; onFocusNode: (nodeId: string) => void; theme: CanvasTheme }) {
    ...
    const filtered = useMemo(() => {
        ...
        return nodes.filter((node) => (typeFilter === "all" || node.type === typeFilter) && (!query || [node.title, node.metadata?.content, node.metadata?.prompt, nodePreviewText(node, production)].filter(Boolean).join(" ").toLowerCase().includes(query)));
    }, [nodes, keyword, production, typeFilter]);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="min-w-0 flex-1 space-y-0.5">
                                        ...
                                        <span className="block truncate text-xs leading-snug opacity-50">{nodePreviewText(node, production)}</span>
                                    </span>
        ...
        try {
            for (const file of files) {
                if (file.type.startsWith("image/")) {
                    const image = await uploadImage(file, { kind: "library", ownerId: "library" });
                    await addAsset({ kind: "image", title: file.name || "图片", coverUrl: image.url, tags: [], data: { dataUrl: image.url, storageKey: image.storageKey, width: image.width, height: image.height, bytes: image.bytes, mimeType: image.mimeType } });
                    added += 1;
                } else if (file.type.startsWith("video/")) {
                    const media = await uploadMediaFile(file, "video", { kind: "library", ownerId: "library" });
                    await addAsset({ kind: "video", title: file.name || "视频", coverUrl: "", tags: [], data: { url: media.url, storageKey: media.storageKey, width: media.width || 0, height: media.height || 0, bytes: media.bytes, mimeType: media.mimeType } });
                    added += 1;
                }
            }
                                        ...
                                        <div className="grid grid-cols-2 gap-2 px-1 pb-2 pt-1">
                                            {group.items.map((asset) => (
                                                <AssetCard
                                                    ...
                                                    onRemove={() => void removeAsset(asset.id).then(() => message.success("资产已移除")).catch((error) => message.error(error instanceof Error ? error.message : "移除资产失败"))}
                                                />
                                            ))}
                                        </div>
                                    )}
