        ...
        <>
            <div className="pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between pl-1 pr-4">
                <div className="pointer-events-auto flex min-w-0 items-center gap-2">
                    ...
                    <span className="text-xs text-stone-500">{saveStatus === "saving" ? "正在保存…" : saveStatus === "error" ? "保存失败" : saveStatus === "conflict" ? "保存冲突" : saveStatus === "saved" ? "已保存" : ""}</span>
            ...
            <Modal title="快捷键" open={shortcutsOpen} onCancel={() => setShortcutsOpen(false)} footer={null} centered>
                <div className="space-y-2 border-t pt-4 text-sm" style={{ borderColor: theme.node.stroke }}>
                    <Shortcut keys={["空白左键拖动"]} value="框选节点" />
                    <Shortcut keys={["Shift", "空白左键拖动"]} value="追加框选" />
                    <Shortcut keys={["Space", "左键拖动"]} value="平移视图" />
                    <Shortcut keys={["鼠标中键拖动"]} value="平移视图" />
                    <Shortcut keys={["触控板双指滑动"]} value="平移视图" />
...
function CompactAgentStatus({ status, onClick }: { status: { connected: boolean; enabled: boolean; activity: string }; onClick: () => void }) {
    const colorTheme = useThemeStore((state) => state.theme);
    const theme = canvasThemes[colorTheme];
    const label = status.connected ? "Hermes 已连接" : status.enabled ? `Hermes ${status.activity || "连接中"}` : "Hermes 未连接";
    const dotColor = status.connected ? "#22c55e" : status.enabled ? "#f59e0b" : theme.node.muted;
    return (
        <button type="button" className="flex h-8 items-center gap-1.5 text-xs transition hover:opacity-75" style={{ color: status.connected ? "#16a34a" : status.enabled ? "#d97706" : theme.node.muted }} onClick={onClick} title="打开本地 Hermes 面板">
