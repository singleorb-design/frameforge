...
export async function exportCanvasProjects(projects: CanvasProject[], fileName = "影格工坊") {
    ...
    const data: CanvasExportFile = { app: "frameforge", version: 4, exportedAt: new Date().toISOString(), projects: exportedProjects };
...
export async function exportCanvasNodes(nodes: CanvasNodeData[], fileName = "画布元素") {
    ...
    await Promise.all(
        nodes.map(async (node) => {
            ...
            if (node.type === CanvasNodeType.Script || node.type === CanvasNodeType.MarkdownDocument) return void zipFiles.push({ name: uniqueName(title, "md"), data: node.metadata?.content || "" });
