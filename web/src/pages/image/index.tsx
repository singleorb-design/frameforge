...
const logStore = createWorkbenchRecordStore("image");
...
export default function ImagePage() {
    ...
    useEffect(() => {
        setStorageScope({ kind: "workbench", ownerId: "image" });
    ...
    const deleteSelectedLogs = async () => {
        ...
        await localApi.cleanupMedia().catch((error) => console.error("清理生成图片失败", error));
        await refreshLogs();
    ...
    const saveLog = (log: GenerationLog) => {
        void logStore
            .setItem(log.id, serializeLog(log))
            .then(refreshLogs)
            .catch((error) => message.error(error instanceof Error ? error.message : "生成记录保存失败"));
    };
