...
import { createEmptyProductionProject } from "@/types/production";
...
type PluginHostParams = {
    ...
    productionRef?: MutableRefObject<ProductionProject>;
...
export function usePluginHost(params: PluginHostParams) {
    const { effectiveConfig, isAiConfigReady, openConfigDialog, theme, nodesRef, connectionsRef, productionRef, viewportRef, setNodes, setDialogNodeId, applyAgentOps } = params;
    const emptyProduction = useMemo(() => createEmptyProductionProject(), []);
    ...
    const pluginHost = useMemo<CanvasPluginHost>(
        () => ({
            ...
            getProduction: () => productionRef?.current || emptyProduction,
            getUpstream: (nodeId) =>
