...
import { MINIMAX_BASE_URL, MINIMAX_IMAGE_GENERATION_SCRIPT, MINIMAX_IMAGE_MODELS, MINIMAX_TEXT_CHAT_COMPLETIONS_SCRIPT, MINIMAX_TEXT_MODELS } from "@/lib/minimax";
...
export const CONFIG_STORE_KEY = "frameforge:ai_config_store";
...
export function createMiniMaxTextChannel(channel?: Partial<ModelChannel>): ModelChannel {
    return createModelChannel({
        id: channel?.id,
        name: channel?.name || "MiniMax 文本",
        baseUrl: channel?.baseUrl || MINIMAX_BASE_URL,
        apiKey: channel?.apiKey || "",
        apiFormat: "openai",
        models:
            channel?.models ||
            MINIMAX_TEXT_MODELS.map((name) => ({
                name,
                capability: "text" as const,
                script: MINIMAX_TEXT_CHAT_COMPLETIONS_SCRIPT,
            })),
    });
}

export function createMiniMaxImageChannel(channel?: Partial<ModelChannel>): ModelChannel {
    return createModelChannel({
        id: channel?.id,
        name: channel?.name || "MiniMax 生图",
        baseUrl: channel?.baseUrl || MINIMAX_BASE_URL,
        apiKey: channel?.apiKey || "",
        apiFormat: "openai",
        models:
            channel?.models ||
            MINIMAX_IMAGE_MODELS.map((name) => ({
                name,
                capability: "image" as const,
                script: MINIMAX_IMAGE_GENERATION_SCRIPT,
            })),
    });
}
