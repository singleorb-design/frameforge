...
export const useAgentStore = create<AgentStore>((set, get) => ({
    ...
    setAgentState: (patch) => {
        set(patch);
        if ("width" in patch || "url" in patch || "token" in patch) scheduleAgentPreferences();
    },
...
export async function hydrateAgentPreferences() {
    const saved = await getAppPreference("canvas-agent", { width: 440, url: "http://127.0.0.1:17371", token: "" });
    useAgentStore.setState({ width: Number(saved.width) || 440, url: saved.url || "http://127.0.0.1:17371", token: saved.token || "" });
}

function scheduleAgentPreferences() {
    if (preferenceTimer) clearTimeout(preferenceTimer);
    preferenceTimer = setTimeout(() => {
        preferenceTimer = null;
        const { width, url, token } = useAgentStore.getState();
        void setAppPreference("canvas-agent", { width, url, token });
    }, 300);
}
