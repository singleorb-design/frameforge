...
export default defineConfig({
    ...
    server: {
        ...
        proxy: {
            "/local-api": {
                target: `http://127.0.0.1:${process.env.LOCAL_STORAGE_PORT || "17372"}`,
                configure(proxy) {
                    proxy.on("proxyReq", (request) => request.setHeader("x-local-storage-token", process.env.LOCAL_STORAGE_TOKEN || ""));
                },
            },
        },
    },
