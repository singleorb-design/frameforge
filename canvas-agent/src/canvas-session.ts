...
export class CanvasSession {
    ...
    private agentState: AgentState = { busy: false, threadId: "", turnId: "" };
    ...
    health() {
        return { ok: true, hasCanvas: Boolean(this.canvasState), clients: this.clients.size, agentBusy: this.agentState.busy };
    }

    get agentBusy() {
        return this.agentState.busy;
    }

    setAgentState(patch: Partial<AgentState>) {
        this.agentState = { ...this.agentState, ...patch };
        this.emitAll("agent_state", this.agentState);
    }
    ...
    openEvents(url: URL, res: ServerResponse) {
        ...
        sendEvent(res, "hello", { ok: true, clientId, agent: this.agentState });
