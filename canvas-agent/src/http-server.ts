...
import { deleteHermesSession, interruptHermesTurn, listHermesSessions, readHermesSession, resumeHermesSession, runHermesTurn, startHermesSession } from "./hermes-acp.js";
...
export function startHttpServer() {
    ...
    app.get("/agent/hermes/workspace", (_req, res) => {
        const workspace = ensureSiteWorkspace(config);
        res.json({ ok: true, workspace });
    });
    app.get("/agent/hermes/threads", route(async (req, res) => {
        const workspace = ensureSiteWorkspace(config);
        const result = await listHermesSessions(emit, workspace.workspacePath);
        res.json({ ok: true, workspace, ...result });
    }));
    app.post("/agent/hermes/threads/new", route(async (_req, res) => {
        if (session.agentBusy) return res.status(409).json({ ok: false, error: "Hermes 正在运行，请等待当前任务完成" });
        const workspace = ensureSiteWorkspace(config);
        const thread = await startHermesSession(emit, workspace.workspacePath);
        const activeThreadId = thread.id;
        const nextWorkspace = setActiveThread(activeThreadId, { emptyThread: true });
        res.json({ ok: true, workspace: nextWorkspace, thread, messages: [] });
    }));
    app.get("/agent/hermes/threads/:threadId", route(async (req, res) => {
        const workspace = ensureSiteWorkspace(config);
        const threadId = routeParam(req.params.threadId);
        res.json({ ok: true, workspace, ...(await readHermesSession(emit, threadId, workspace.workspacePath)) });
    }));
    app.post("/agent/hermes/threads/:threadId/resume", route(async (req, res) => {
        if (session.agentBusy) return res.status(409).json({ ok: false, error: "Hermes 正在运行，请等待当前任务完成" });
        const workspace = ensureSiteWorkspace(config);
        const threadId = routeParam(req.params.threadId);
        const result = await resumeHermesSession(emit, threadId, workspace.workspacePath);
        const nextWorkspace = setActiveThread(threadId);
        res.json({ ok: true, workspace: nextWorkspace, ...result });
    }));
    app.post("/agent/hermes/threads/:threadId/delete", route(async (req, res) => {
        if (session.agentBusy) return res.status(409).json({ ok: false, error: "Hermes 正在运行，请等待当前任务完成" });
        const workspace = ensureSiteWorkspace(config);
        const threadId = routeParam(req.params.threadId);
        await deleteHermesSession(emit, threadId);
        setActiveThread(workspace.activeThreadId === threadId ? "" : workspace.activeThreadId || "");
        res.json({ ok: true });
    }));
    app.post("/agent/hermes/turn", route(async (req, res) => {
        if (session.agentBusy) return res.status(409).json({ ok: false, error: "Hermes 正在运行，请等待当前任务完成" });
        ...
        session.setAgentState({ busy: true, threadId: String(req.body?.threadId || workspace.activeThreadId || ""), turnId: "" });
        try {
            ...
            void runHermesTurn(withAttachmentContext(prompt, attachmentRefs), attachments, {
                sessionId: threadId,
                cwd: workspace.workspacePath,
                emit: turnEmit,
                onStart: clientId ? () => session.bindClient(clientId) : undefined,
                onSession: (actualThreadId) => {
                    if (actualThreadId !== threadId) {
                        threadId = actualThreadId;
                        setActiveThread(threadId, { emptyThread: true });
                    }
                    session.setAgentState({ busy: true, threadId, turnId: "" });
                ...
                onFinish: () => {
                    ...
                    session.setAgentState({ busy: false, threadId, turnId: "" });
                },
            });
            res.json({ ok: true, threadId });
        } catch (error) {
            session.setAgentState({ busy: false, threadId: String(req.body?.threadId || workspace.activeThreadId || ""), turnId: "" });
            throw error;
        }
    }));
    app.post("/agent/hermes/interrupt", route(async (req, res) => {
        const ok = await interruptHermesTurn(emit, String(req.body?.threadId || ""));
        res.json({ ok });
    }));
    ...
    app.listen(port, "127.0.0.1", () => {
        console.log("FrameForge Canvas Agent");
        console.log(`Local URL: ${config.url}`);
        console.log(`Connect token: ${config.token}`);
        console.log("Sidebar agent: Hermes ACP (hermes acp)");
        console.log("MCP endpoint remains available: canvas-agent mcp");
    });
}
