...
本地 Agent 连接画布的核心不是让 Agent 直接控制浏览器，也不是模拟鼠标点击或操作 DOM，而是在用户电脑上运行一个 `Canvas Agent` 作为桥接服务。
...
真正的画布数据仍然在浏览器里，节点、连线、选区和视口都由前端 React 状态维护。Agent 只能通过 `Canvas Agent` 暴露的工具协议发出“读取画布”或“执行画布操作”的请求。
...
用户在本地 Codex 里发起请求，例如读取画布、创建文本节点、连接节点或触发生成。Codex 调用 `frameforge` MCP 工具，MCP 工具再请求本机 `Canvas Agent` 的 `/api/tools` 接口。
...
用户在网页右侧面板输入提示词后，浏览器把请求发送到 `Canvas Agent` 的 `/agent/hermes/turn` 接口。`Canvas Agent` 会在本机启动或复用 `hermes acp`，并给这个 Hermes 会话注入同一套 `frameforge` MCP 工具。

所以即使入口在浏览器右侧面板，真正改动画布时仍然会回到同一条工具链路：

```txt
Hermes -> MCP -> Canvas Agent -> 浏览器画布
```

右侧面板只是把“打开 Hermes 会话、发送 prompt、展示流式事件”这件事搬到了网页里。
...
`Canvas Agent` 把最近一次快照保存在内存里。Agent 调用 `canvas_get_state` 时，读取的是这份由浏览器同步过来的最新快照，而不是直接读取浏览器 DOM。
...
```txt
Agent 调 canvas_create_text_node
  -> MCP POST /api/tools
...
为了避免 Agent 直接改动画布，浏览器侧对 `canvas_apply_ops` 默认有二次确认机制。收到写操作后，右侧面板会先展示待执行工具调用，用户确认后才真正应用到画布。

执行后，前端会保存最近一次操作前的画布快照，用于撤销上一轮 Agent 工具操作。

如果关闭工具确认，本地 Agent 可以直接执行画布工具调用，但仍然是通过同一套 `Canvas Agent -> SSE -> 浏览器执行` 的链路。
...
- Agent 不直接访问浏览器 DOM，也不直接持有画布内部状态。
- 线上网页不保存本机 Hermes 登录态、API Key 或本地文件权限。
...
```txt
Agent 调 MCP 工具，Canvas Agent 做本机桥接，浏览器通过 SSE 接收工具调用并在 React 状态中真正修改画布。
```

读状态是：

```txt
浏览器快照 -> Canvas Agent 内存 -> MCP 工具 -> Agent
```

写操作是：

```txt
Agent -> MCP 工具 -> Canvas Agent -> SSE tool_call -> 浏览器 applyOps -> /canvas/result -> Agent
```
