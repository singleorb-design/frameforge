---
title: Docker 部署
description: 使用 Docker Compose 部署影格工坊
---
...
当前镜像只启动静态前端，尚未包含 Bun 本机存储服务和 `data/` 持久化挂载，因此不支持当前完整画布模式。需要画布、素材、生成记录、API Key 和原始媒体持久化时，请在仓库根目录执行 `make`。
...
services:
  app:
    image: ghcr.io/basketikun/frameforge:latest
    ports:
      - "3000:3000"
    environment:
...
docker run -d -p 3000:3000 \
  ...
  ghcr.io/basketikun/frameforge:latest
```
